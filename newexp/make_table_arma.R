...
for(cter in 1:3) for(tter in 1:2) for(ater in 1:3) for(oter in 1:5) for(kter in 1:4){
  tmp = NULL; for(est in 1:npar){
    tmp = cbind(tmp,ret[,cter,tter,ater,oter,kter,est] - arma[est])
  }
  # total bias
...
tab1 = make_table(tmptab, iT=1, cont=1:3, outlier=2:5, alpha=1:3,kappa=1:4)
tab2 = make_table(tmptab, iT=2, cont=1:3, outlier=2:5, alpha=1:3,kappa=1:4)

tables = data.frame(tab2)
tables = cbind(b=c(5,"","",10,"","",50,"","",100,"",""), a=c(1,5,10), tables)

kbl(tables, digits=4, booktabs=T, linesep="") %>%
  add_header_above(c(" ","%", rep(c("k=0","2","5","9"),3))) %>%
  add_header_above(c("z", "a", "no"=4, "AO"=4, "IO"=4)) %>%
